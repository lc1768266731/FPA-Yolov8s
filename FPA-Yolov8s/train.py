...
if __name__ == "__main__":
    # 加载模型
    # model = YOLO(r'yolov8.yaml')#训练什么模型
    # 训练参数 ----------------------------------------------------------------------------------------------
    model.train(
        data="VisDrone.yaml", #数据集路径
        epochs=200,  # (int) 训练的周期数
        patience=0,  # (int) 等待无明显改善以进行早期停止的周期数
        batch=10, # (int) 每批次的图像数量（-1 为自动批处理）
        imgsz=640,  # (int) 输入图像的大小，整数或w，h
        val=True,  # (bool) 在训练期间进行验证/测试s
        ...
        name="FPA-yolov8s",  # (str, optional) 实验名称，结果保存在'project/name'目录下
